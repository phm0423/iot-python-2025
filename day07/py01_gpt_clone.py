...
# 6. 제미나이API용 구성
genai.configure(api_key='AIzaSyB_mhxbmGpU1yVYuRDqQcb84KOj7nwESWk')  # 신청한 API키
...
def responseMessage():
    ...
    print(inputText)
    textMessage.delete('1.0', END)  # 입력창에 글 지우기
...
# 12. 아이콘 변경
root.iconbitmap('chat.ico')   # pyinstaller로 설치할땐 해당폴더에 복사하고 경로변경
...
# 8. 입력창에서 엔터를 치면 key press 이벤트처리함수 발생
textMessage.bind('<Key>', keypress)
textMessage.pack(side=LEFT, padx=15)

buttonSend = Button(inputFrame, width=10, text='전송', bg='green', fg='white', 
                    font = myFont, 
                    command=responseMessage)
buttonSend.pack(side=RIGHT, padx=20, pady=5)

# 5. API 호출 결과메세지 출력될 스크롤기능 텍스트위젯
textResult = ScrolledText(root, wrap=WORD, bg='#000000', fg='#FFFFFF', font=myFont)    # bg='black', fg='white'
